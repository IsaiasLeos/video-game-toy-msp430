...
typedef struct AbTriangle {
  void (*getBounds)(const struct AbTriangle *shape, const Vec2 *centerPos, Region *bounds);
  int (*check)(const struct AbTriangle *shape, const Vec2 *centerPos, const Vec2 *pixelLoc);
  int size;
} AbTriangle;
